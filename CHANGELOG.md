...
- Pre-commit hooks (`.pre-commit-config.yaml`): file hygiene, `uv-lock`, local `ruff`/`mypy` via `uv run`, and `pytest` on pre-push.
- `pre-commit` dev dependency; CI and publish workflows run `pre-commit run --all-files`.
